...
// French description of every perception/property tag (TDD §2) — DERIVED from
// content/tags.json (single source, edited in the atelier's Tags tab). The
// inspector GENERATES organ descriptions from these; gauge-shaped tags (forme
// "jauge") show their stack count next to the label.
import TAGS_CATALOG from '../content/tags.json';

export const TAG_FR = Object.fromEntries(Object.entries(TAGS_CATALOG.tags ?? {}).map(([id, d]) => [
  id,
  d.forme === 'jauge' ? { label: d.label ?? id, gauge: d.description ?? '' }
                      : { label: d.label ?? id, desc: d.description ?? '' },
]));
