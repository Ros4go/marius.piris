...
> **⚠ État d'implémentation.** Les patterns signatures ci-dessus — et les comportements de mob (rampant/embusqué/distant/nuée…) — ne sont **pas encore branchés** sur le moteur de combat temps-réel : ils vivaient sur un ancien chemin tour-par-tour, depuis retiré du code. Pour l'instant, boss et mobs combattent via les **skills génériques de leurs organes** ; seule la **reconfiguration de barre en phase 2** est active. Cette section reste la cible de design à réimplémenter dans le BattleEngine. *(cf. `CHAIR_SPEC.md` §7)*
